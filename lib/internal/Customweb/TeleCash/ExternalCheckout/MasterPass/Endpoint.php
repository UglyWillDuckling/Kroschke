 ...
 */
class Customweb_TeleCash_ExternalCheckout_MasterPass_Endpoint extends Customweb_Payment_ExternalCheckout_AbstractCheckoutEndpoint {
	...
	/**
	 *
	 * @var Customweb_TeleCash_Container
	 */
	private $container;
	...
	public function redirectAction(Customweb_Core_Http_IRequest $request){
		...
		try {
			...
			$templateContext->addVariable('url', 
					Customweb_Core_Url::_($this->container->getConfiguration()->getBaseUrl() . Customweb_TeleCash_IConstants::URL_CONNECT)->toString());
	...
	public function confirmationAction(Customweb_Core_Http_IRequest $request){
		...
		try {
				...
				if ($additionalFormErrorMessage === null) {
					try {
						...
						if (!$transaction->isAuthorized() && !$transaction->isAuthorizationFailed()) {
							...
							$parameterBuilder = new Customweb_TeleCash_ExternalCheckout_MasterPass_ConfirmationParameterBuilder($this->getContainer(),
			...
			$templateContext->addVariable('javascript', 
					$this->getAjaxJavascript('.telecash-masterpass-shipping-pane', '.telecash-masterpass-confirmation-pane'));
		...
		if ($context->getState() == Customweb_Payment_ExternalCheckout_IContext::STATE_COMPLETED) {
			...
			for ($i = 0; $i < 5; $i++) {
				try {
					...
					$authorizationHandler = $this->container->getBean('Customweb_TeleCash_Authorization_AuthorizationHandler');
	...
	private function getRedirectionParameters(Customweb_Payment_ExternalCheckout_IContext $context, $token){
		$builder = new Customweb_TeleCash_ExternalCheckout_MasterPass_RedirectionParameterBuilder($context, $this->container, $token);
	...
	/**
	 *
	 * @return Customweb_TeleCash_Debitor_Manager
	 */
	protected function getDebitorManager(){
		return $this->getContainer()->getBean('Customweb_TeleCash_Debitor_Manager');
	}

	/**
	 *
	 * @return Customweb_TeleCash_Method_Factory
	 */
	protected function getMethodFactory(){
		return $this->getContainer()->getBean('Customweb_TeleCash_Method_Factory');
	}

	protected function getPaymentMethodByTransaction(Customweb_TeleCash_Authorization_Transaction $transaction){
