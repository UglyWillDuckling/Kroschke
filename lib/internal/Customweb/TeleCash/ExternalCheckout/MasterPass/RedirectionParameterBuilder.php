...
class Customweb_TeleCash_ExternalCheckout_MasterPass_RedirectionParameterBuilder extends Customweb_TeleCash_AbstractParameterBuilder {
	...
	protected function getPaymentMethod(){
		$factory = $this->getContainer()->getBean('Customweb_TeleCash_Method_Factory');
	...
	public function build(){
		...
		$parameters['hashExtended'] = Customweb_TeleCash_Util::calculateExtendedHash($parameters,$this->getConfiguration()->getConnectSharedSecret());
		return $parameters;
	}
	...
	protected function getPaymentParameters(){
		...
		$parameters['currency'] = Customweb_TeleCash_Util::getNumericCurrencyCode($this->getCheckoutContext()->getCurrencyCode());
		return $parameters;
	}
	...
	protected function getOperationModeParameter(){
		...
		$captureType = Customweb_TeleCash_IConstants::OPERATION_SALE;
		if (strtolower($capturingMode) == 'deferred') {
			$captureType = Customweb_TeleCash_IConstants::OPERATION_AUTHORISATION;
		}
		return array(
			'txntype' => $captureType,
			'mode' => Customweb_TeleCash_IConstants::DATA_ALL,
			'reviewOrder' => 'true',
	...
	protected function getLanguageParameter(){
		...
		$language = Customweb_Core_Util_Language::getCleanLanguageCode($this->getCheckoutContext()->getLanguage(), 
				Customweb_TeleCash_Util::$supportedLanguages);
