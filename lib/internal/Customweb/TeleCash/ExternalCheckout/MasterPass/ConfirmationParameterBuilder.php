...
class Customweb_TeleCash_ExternalCheckout_MasterPass_ConfirmationParameterBuilder extends Customweb_TeleCash_AbstractParameterBuilder {
	
	private $transaction;
	
	private $providerData;

	public function __construct(Customweb_DependencyInjection_IContainer $container, Customweb_TeleCash_Authorization_Transaction $transaction, array $providerData){
	...
	public function build(){
		...
		$parameters['currency'] = Customweb_TeleCash_Util::getNumericCurrencyCode($this->getOrderContext()->getCurrencyCode());
	...
	protected function getPaymentParameters(){
		...
		$parameters['currency'] = Customweb_TeleCash_Util::getNumericCurrencyCode($this->getTransaction()->getCurrencyCode());
		return $parameters;
	}
}