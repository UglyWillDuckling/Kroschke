 ...
 * @Bean
 */
class Customweb_TeleCash_Authorization_Server_Adapter extends Customweb_TeleCash_Authorization_AbstractAdapter implements 
	...
	public function processAuthorization(Customweb_Payment_Authorization_ITransaction $transaction, array $parameters){
		if (!($transaction instanceof Customweb_TeleCash_Authorization_Transaction)) {
		}
		try {
			...
			$service = new Customweb_TeleCash_SoapService($this->getAPIUser(false), $this->getAPIPassword(false), $this->getAPIUrl());
			$service->setClientCertificate(Customweb_TeleCash_Util::getCertificate($this->getConfiguration(), false));
			...
			$authorizationHandler = $this->getContainer()->getBean('Customweb_TeleCash_Authorization_AuthorizationHandler');
