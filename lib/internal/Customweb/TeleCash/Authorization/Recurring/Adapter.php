 ...
 * @Bean
 */
class Customweb_TeleCash_Authorization_Recurring_Adapter extends Customweb_TeleCash_Authorization_AbstractAdapter implements 
	...
	public function createTransaction(Customweb_Payment_Authorization_Recurring_ITransactionContext $transactionContext){
		$transaction = new Customweb_TeleCash_Authorization_Transaction($transactionContext);
	...
	public function process(Customweb_Payment_Authorization_ITransaction $transaction){
		try {
			...
			$urlString = $this->getBaseUrl() . Customweb_TeleCash_IConstants::URL_API;
			$orderId = Customweb_TeleCash_Util::getOrderAppliedSchema($transaction, $this->getConfiguration());
			
			//Create XML
			$ipgFunction = Customweb_TeleCash_Stubs_Com_IpgOnline_Ipgapi_Schemas_A1_Function::INSTALL();
			$ipgRecurringDetails = new Customweb_TeleCash_Stubs_Com_IpgOnline_Ipgapi_Schemas_A1_RecurringPaymentInformation();
			$ipgRecurringDetails->setRecurringStartDate(
					Customweb_TeleCash_Stubs_Com_IpgOnline_Ipgapi_Schemas_A1_StringDate::_()->set($date))->setInstallmentCount(
					Customweb_TeleCash_Stubs_Com_IpgOnline_Ipgapi_Schemas_A1_RecurringPaymentInformation_InstallmentCount::_()->set(1))->setInstallmentFrequency(
					Customweb_TeleCash_Stubs_Com_IpgOnline_Ipgapi_Schemas_A1_RecurringPaymentInformation_InstallmentFrequency::_()->set(1))->setInstallmentPeriod(
					Customweb_TeleCash_Stubs_Com_IpgOnline_Ipgapi_Schemas_A1_RecurringPaymentInformation_InstallmentPeriod::DAY());
			...
			$ipgPayment->setCurrency(Customweb_TeleCash_Stubs_Com_IpgOnline_Ipgapi_Schemas_V1_CurrencyType::_()->set($currency))->setChargeTotal(
					Customweb_TeleCash_Stubs_Com_IpgOnline_Ipgapi_Schemas_V1_AmountValueType::_()->set($formattedAmount))->setHostedDataID(
					Customweb_TeleCash_Stubs_Com_IpgOnline_Ipgapi_Schemas_V1_String128max::_()->set($initalAlias));
			
			$ipgRecurringPayment = new Customweb_TeleCash_Stubs_Com_IpgOnline_Ipgapi_Schemas_A1_RecurringPayment();
			$ipgRecurringPayment->setRecurringPaymentInformation($ipgRecurringDetails)->setFunction($ipgFunction)->setPayment($ipgPayment)->setOrderId(
					Customweb_TeleCash_Stubs_Com_IpgOnline_Ipgapi_Schemas_V1_String100max::_()->set($orderId));
			if ($isMoto) {
				$ipgRecurringPayment->setTransactionOrigin(
						Customweb_TeleCash_Stubs_Com_IpgOnline_Ipgapi_Schemas_A1_TransactionOrigin::MOTO());
			}
			else {
				$ipgRecurringPayment->setTransactionOrigin(
						Customweb_TeleCash_Stubs_Com_IpgOnline_Ipgapi_Schemas_A1_TransactionOrigin::ECI());
			}
			
			$ipgAction = new Customweb_TeleCash_Stubs_Com_IpgOnline_Ipgapi_Schemas_A1_Action();
			$ipgAction->setRecurringPayment($ipgRecurringPayment);
			
			$actionRequest = new Customweb_TeleCash_Stubs_Com_IpgOnline_Ipgapi_Schemas_Ipgapi_IPGApiActionRequest();
			$actionRequest->setAction($ipgAction);
			
			$service = new Customweb_TeleCash_SoapService($this->getAPIUser($isMoto), $this->getAPIPassword($isMoto), $this->getAPIUrl());
			$service->setClientCertificate(Customweb_TeleCash_Util::getCertificate($this->getConfiguration(), $isMoto));
			...
			try {
				...
				if ($e->getFaultCode() == 'SOAP-ENV:Server') {
					$errorMessage = 'Server Error! Please contact TeleCash';
				}
				elseif ($e->getFaultCode() == 'SOAP-ENV:Client') {
					...
					$orderResponse = $decoder->decodeFromDom($details, 
							"Customweb_TeleCash_Stubs_Com_IpgOnline_Ipgapi_Schemas_Ipgapi_IPGApiOrderResponse");
					/**
					 * @var Customweb_TeleCash_Stubs_Com_IpgOnline_Ipgapi_Schemas_Ipgapi_IPGApiOrderResponse $orderResponse
					 */
