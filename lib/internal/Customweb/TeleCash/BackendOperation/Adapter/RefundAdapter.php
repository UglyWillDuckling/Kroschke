 ...
 * @Bean
 */
class Customweb_TeleCash_BackendOperation_Adapter_RefundAdapter extends Customweb_TeleCash_AbstractAdapter implements 
	...
	public function partialRefund(Customweb_Payment_Authorization_ITransaction $transaction, $items, $close){
		...
		$ipgTransactionDetails = new Customweb_TeleCash_Stubs_Com_IpgOnline_Ipgapi_Schemas_V1_TransactionDetails();
		$ipgTransactionDetails->setOrderId(
				Customweb_TeleCash_Stubs_Com_IpgOnline_Ipgapi_Schemas_V1_String100max::_()->set($transaction->getPaymentId()));

		//Create XML
		$tDate = $transaction->getCaptureTDate();
		if($tDate != null) {
			$ipgTransactionDetails->setTDate(Customweb_TeleCash_Stubs_Com_IpgOnline_Ipgapi_Schemas_V1_TDate::_()->set($tDate));
		}
		
		$ipgPayment = new Customweb_TeleCash_Stubs_Com_IpgOnline_Ipgapi_Schemas_V1_Payment();
		$ipgPayment->setChargeTotal(Customweb_TeleCash_Stubs_Com_IpgOnline_Ipgapi_Schemas_V1_AmountValueType::_()->set($formattedAmount))->setCurrency(
				Customweb_TeleCash_Stubs_Com_IpgOnline_Ipgapi_Schemas_V1_CurrencyType::_()->set($currency));
		
		$ipgTransaction = new Customweb_TeleCash_Stubs_Com_IpgOnline_Ipgapi_Schemas_V1_TransactionElement();
		...
		Customweb_TeleCash_Util::setTxTypeForPaymentMethod($paymentMethod, Customweb_TeleCash_IConstants::OPERATION_REFUND, 
				$ipgTransaction);
		...
		$orderRequest = new Customweb_TeleCash_Stubs_Com_IpgOnline_Ipgapi_Schemas_Ipgapi_IPGApiOrderRequest();
		...
		$service->setClientCertificate(Customweb_TeleCash_Util::getCertificate($this->getConfiguration(), $transaction->isMoto()));
