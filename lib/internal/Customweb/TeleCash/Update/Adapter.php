 ...
 * @Bean
 */
class Customweb_TeleCash_Update_Adapter extends Customweb_TeleCash_AbstractAdapter implements Customweb_Payment_Update_IAdapter {
	...
	public function updateTransaction(Customweb_Payment_Authorization_ITransaction $transaction){
		/* @var $transaction Customweb_TeleCash_Authorization_Transaction */
		...
		$ipgAction->setInquiryOrder(
				Customweb_TeleCash_Stubs_Com_IpgOnline_Ipgapi_Schemas_A1_InquiryOrder::_()->setOrderId(
						Customweb_TeleCash_Stubs_Com_IpgOnline_Ipgapi_Schemas_V1_String100max::_()->set($oid)));
		...
		if (substr($approvalCode->__toString(), 0, 1) == 'Y') {
			...
			if ($parameters['txntype'] == Customweb_TeleCash_IConstants::OPERATION_SALE) {
