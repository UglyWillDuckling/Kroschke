...
define([
	...
	'Customweb_TeleCashCw/js/checkout',
	'Customweb_TeleCashCw/js/authorizationMethod',
	'Customweb_TeleCashCw/js/alias'
], function(
	$,
	Component,
	mageTemplate,
	...
	return Component.extend({
		defaults: {
			template: 'Customweb_TeleCashCw/payment/telecashcw_direktueberweisung',
			fieldErrorTemplate: '<div for="<%- id %>" generated="true" class="mage-error" id="<%- id %>-error"><%- message %></div>',
			authorizationUrl: url.build('telecashcw/checkout/authorize/')
		},
		...
		getForm: function() {
			...
			if (!customer.isLoggedIn()) {
				serviceUrl = urlBuilder.createUrl('/guest-carts/:cartId/telecashcw/checkout/payment-form/:paymentMethod', {
					cartId: quote.getQuoteId(),
					paymentMethod: this.item.method
				});
			} else {
				serviceUrl = urlBuilder.createUrl('/carts/mine/telecashcw/checkout/payment-form/:paymentMethod', {
					paymentMethod: this.item.method
				});
			}
			storage.get(
