...
namespace Customweb\TeleCashCw\Block\Adminhtml\Checkout;
...
class Payment extends \Magento\Backend\Block\Widget\Form\Container
{
	 ...
	 * @var string
	 */
	protected $_blockGroup = 'Customweb_TeleCashCw';
	...
	protected function _construct()
	{
		...
		$this->setId('telecashcw_checkout_payment');

		$this->buttonList->update('save', 'label', __('Submit Order'));
		$this->buttonList->update('save', 'class', 'primary');
		$this->buttonList->update('save', 'id', 'telecashcw-payment-submit-button');
		...
		$this->buttonList->add(
			...
			[
				'label' => __('Cancel'),
				'class' => 'cancel',
				'id' => 'telecashcw-payment-cancel-button'
			]
		);
	}
	...
	public function getTransaction()
	{
		if (!($this->transaction instanceof \Customweb\TeleCashCw\Model\Authorization\Transaction)) {
			...
			if (!($transaction instanceof \Customweb\TeleCashCw\Model\Authorization\Transaction) || !$transaction->getId()) {
